...
Root imports expose implemented schemas, calculations, application services, errors, and public types. Calculations also have a focused `@aperture/health/calculations` entry, and service composition is available through `@aperture/health/application`. `createHealthService` exposes the complete tested lifecycle and summary inventory as 137 distinct runtime methods. The types-only `@aperture/health/contracts` entry remains a compatibility alias for implemented public types.
