...
describe("feature registry", () => {
  it("publishes generated navigation, searchable routes, widgets, permissions, modules, and migrations", () => {
    expect(generatedFeatureManifests.map(({ id }) => id)).toEqual(["today", "education", "health", "finance", "calculators", "settings"]);
    expect(featureRegistry.navigation("web").map(({ path }) => path)).toEqual(["/today", "/education", "/health", "/finance", "/calculators", "/settings"]);
    expect(featureRegistry.search("assignment", "web").map(({ id }) => id)).toEqual(["education.assignments"]);
    expect(featureRegistry.widgets("mobile").map(({ id }) => id)).toEqual(["education.deadlines", "health.plans", "finance.reminders"]);
  ...
  it("applies enablement overrides without disabling required features", () => {
    expect(featureRegistry.navigation("web", { education: false, today: false }).map(({ featureId }) => featureId)).toEqual(["today", "health", "finance", "calculators", "settings"]);
