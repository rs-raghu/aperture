...
import { allCalculatorPresentations, calculatorPresentationCategories, type CalculatorPresentationCategory } from "@aperture/calculators";
import { FinanceEmptyState, FinancePageHeader, FinancePanel, FinanceSelectInput, FinanceTextInput } from "../components/ui";
import { useFinance } from "../providers/finance-provider";

function CalculatorCard({ calculator }: { readonly calculator: (typeof allCalculatorPresentations)[number] }) {
...
export function CalculatorHubScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return allCalculatorPresentations.filter((item) => (category === "all" || item.category === category) && (!term || `${item.title} ${item.description} ${item.categoryLabel}`.toLocaleLowerCase().includes(term)));
  }, [search, category]);
