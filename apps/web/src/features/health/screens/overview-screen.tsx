...
export function OverviewScreen() {
  ...
  return <>
    <PageHeader eyebrow="Personal record" title="Health overview" description="Review the observations and activity you explicitly recorded. Aperture provides arithmetic summaries without diagnosis, targets, or medical recommendations." />
    ...
    {query.loading || query.data === null ? <LoadingState /> : <>
      ...
      <div className="grid metric-grid">
        ...
        <article className="metric"><span className="metric-label">Workout sessions</span><strong className="metric-value">{query.data.overview.recentWorkoutCount}</strong><span className="metric-detail">recorded sessions</span></article>
        <article className="metric"><span className="metric-label">Upcoming appointments</span><strong className="metric-value">{query.data.overview.upcomingAppointmentCount}</strong><span className="metric-detail">scheduled after now</span></article>
      </div>
      <div className="grid grid-2">
        ...
        <Panel title="Upcoming appointments" description="Scheduled entries based on the application clock.">
