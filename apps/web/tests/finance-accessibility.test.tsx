...
import { financeNavigation } from "@/features/finance/navigation/finance-navigation";
...
describe("Finance accessibility and route surface", () => {
  ...
  it("links every Finance route and displays the credential warning", () => {
    ...
    expect(financeNavigation).toHaveLength(10);
    expect(new Set(financeNavigation.map(({ href }) => href)).size).toBe(10);
    for (const item of financeNavigation) expect(screen.getByRole("link", { name: item.label }).getAttribute("href")).toBe(item.href);
