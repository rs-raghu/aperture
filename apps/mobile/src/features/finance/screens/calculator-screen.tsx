...
import { ActionButton, ChoiceField, ErrorBanner, NavigationCard, PageHeader, Panel, RecordCard, RecordList, Screen, SuccessMessage, TextField, humanize } from "../components/ui";
...
function RegisteredCalculatorScreen({ calculator }: { readonly calculator: CalculatorPresentationDefinition }) {
  ...
  return <Screen testID={`calculator-${calculator.id}-screen`}><NavigationCard href="/calculators" title="Calculator Hub" description="Return to search and categories." /><PageHeader eyebrow={calculator.categoryLabel} title={calculator.title} description={calculator.description} /><ActionButton label={favorites.has(calculator.id) ? "★ Favorite" : "☆ Add favorite"} tone="secondary" pressed={favorites.has(calculator.id)} onPress={() => toggleFavorite(calculator.id)} /><Text style={styles.example}>Example: {calculator.exampleName}</Text><ErrorBanner error={error} />
