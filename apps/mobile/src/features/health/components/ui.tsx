...
import { healthColors as colors, healthSpacing as spacing } from "../styles/tokens";
import type { HealthMobileError } from "../view-models/health-error";

export function Screen({ children, testID }: { readonly children: ReactNode; readonly testID?: string | undefined }) {
...
export function PageHeader({ title, description }: { readonly title: string; readonly description: string }) {
  return <View style={styles.header} accessibilityRole="header"><Text style={styles.title}>{title}</Text><Text style={styles.description}>{description}</Text></View>;
}

export function PreviewNotice() {
  return <View style={styles.notice} accessibilityRole="summary"><Text style={styles.noticeTitle}>Private workspace</Text><Text style={styles.noticeText}>Signed-in health data is saved to the owner-scoped cloud database. It is a personal record, not medical advice.</Text></View>;
}

export function Panel({ title, description, children }: { readonly title: string; readonly description?: string | undefined; readonly children: ReactNode }) {
...
export function ChoiceField<TValue extends string>({ label, value, options, onChange, error, required }: { readonly label: string; readonly value: TValue; readonly options: readonly ChoiceOption<TValue>[]; readonly onChange: (value: TValue) => void; readonly error?: string | undefined; readonly required?: boolean | undefined }) {
...
export function ActionButton({ label, onPress, pending = false, disabled = false, tone = "primary" }: { readonly label: string; readonly onPress: () => void; readonly pending?: boolean | undefined; readonly disabled?: boolean | undefined; readonly tone?: "primary" | "secondary" | "danger" | undefined }) {
  const unavailable = pending || disabled;
  return <Pressable accessibilityRole="button" accessibilityLabel={pending ? `${label}, in progress` : label} accessibilityState={{ disabled: unavailable, busy: pending }} disabled={unavailable} onPress={onPress} style={[styles.button, tone === "primary" ? styles.buttonPrimary : tone === "danger" ? styles.buttonDanger : styles.buttonSecondary, unavailable ? styles.disabled : null]}>{pending ? <ActivityIndicator color={tone === "secondary" ? colors.ink : "#ffffff"} /> : null}<Text style={tone === "secondary" ? styles.buttonSecondaryText : styles.buttonText}>{pending ? "Working…" : label}</Text></Pressable>;
}
...
export function LoadingState() {
  return <View style={styles.loading} accessibilityRole="progressbar" accessibilityLabel="Loading Health data"><ActivityIndicator color={colors.primary} /><Text style={styles.muted}>Loading Health data…</Text></View>;
}

export function EmptyState({ title, description, href, action }: { readonly title: string; readonly description: string; readonly href?: string | undefined; readonly action?: string | undefined }) {
...
export function Metric({ label, value, detail }: { readonly label: string; readonly value: string; readonly detail: string }) {
  return <View style={styles.metric} accessible accessibilityLabel={`${label}: ${value}. ${detail}`}><Text style={styles.metricLabel}>{label}</Text><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricDetail}>{detail}</Text></View>;
}

export function RecordCard({ title, details, status, children }: { readonly title: string; readonly details: readonly string[]; readonly status?: string | undefined; readonly children?: ReactNode | undefined }) {
  return <View style={styles.record}><View style={styles.recordHeader}><Text style={styles.recordTitle} numberOfLines={3}>{title}</Text>{status ? <StatusBadge value={status} /> : null}</View>{details.map((detail, index) => <Text key={`${String(index)}-${detail}`} style={styles.recordDetail}>{detail}</Text>)}{children ? <View style={styles.actions}>{children}</View> : null}</View>;
}
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, safeArea: { flex: 1, backgroundColor: colors.canvas }, screen: { padding: spacing.lg, paddingBottom: 56, gap: spacing.lg, width: "100%", maxWidth: 760, alignSelf: "center" },
  header: { gap: spacing.sm, paddingTop: spacing.sm }, title: { color: colors.ink, fontSize: 30, lineHeight: 36, fontWeight: "800" }, description: { color: colors.muted, fontSize: 16, lineHeight: 24 },
  notice: { backgroundColor: colors.warningSurface, borderColor: "#e5bd68", borderWidth: 1, borderRadius: 14, padding: spacing.md, gap: spacing.xs }, noticeTitle: { color: colors.warning, fontWeight: "800" }, noticeText: { color: colors.ink, lineHeight: 20 },
  ...
  empty: { alignItems: "center", padding: spacing.xl, gap: spacing.sm }, emptyMark: { color: colors.primary, fontSize: 34 }, emptyTitle: { color: colors.ink, fontWeight: "800", fontSize: 18, textAlign: "center" }, badge: { alignSelf: "flex-start", backgroundColor: "#d9f0eb", borderColor: "#8bc7bd", borderWidth: 1, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 4 }, badgeText: { color: "#075f55", fontSize: 12, fontWeight: "800", textTransform: "capitalize" },
  metric: { minWidth: 145, backgroundColor: "#e8f3f0", borderRadius: 14, padding: spacing.md, gap: spacing.xs }, metricLabel: { color: colors.muted, fontWeight: "700" }, metricValue: { color: colors.ink, fontWeight: "900", fontSize: 24 }, metricDetail: { color: colors.muted, fontSize: 12 }, record: { borderColor: colors.border, borderWidth: 1, borderRadius: 14, padding: spacing.md, gap: spacing.sm, backgroundColor: colors.surface }, recordList: { gap: spacing.md, paddingBottom: spacing.xs }, recordListItem: { width: 280 }, recordHeader: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", gap: spacing.sm }, recordTitle: { color: colors.ink, fontSize: 17, lineHeight: 23, fontWeight: "800", flex: 1 }, recordDetail: { color: colors.muted, lineHeight: 20 }, actions: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm, marginTop: spacing.xs },
